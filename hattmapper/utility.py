...
from .paulihedral import pauliString, depth_oriented_scheduling, synth
...
def paulihedral_transpile(
    hamiltonian: SparsePauliOp, basis_gates: list[str] = ["u3", "cx"], time: float = 1.0
        ...
        for pauli in hamiltonian:
            ...
            terms.append([pauliString(labels, 0, coeff.real, coeff.imag)])
        
        scheduled = depth_oriented_scheduling(
            terms, len(terms[0][0].ps) // 2, 30
        )
        circuit = synth.block_opt_FT(scheduled, time_parameter=time) # type: ignore
        qiskit_circuit = transpile(circuit, basis_gates=basis_gates, optimization_level=3)
        return (qiskit_circuit, permutation)
...
class PaulihedralDriver:
    def __init__(
        ...
        self.circuit, self.permutation = paulihedral_transpile(
            self.mapper.map(self.hamiltonian), # type: ignore
...
class RustiqDriver:
    def __init__(
        ...
        self.rustiq_transpile(self.mapper.map(self.hamiltonian))  # type: ignore

    def synthesis(
        self,
        pauli_sequence: list[str],
        num_qubits: int,
    ):
        circuit = self.circuit = QuantumCircuit(num_qubits)  # type: ignore
        for gate, qubits in pauli_network_synthesis(
            pauli_sequence,
            Metric.DEPTH if self.metric == "depth" else Metric.COUNT,
            False,
        ):
            if gate == "CNOT":
        ...
        return transpile(self.circuit, optimization_level=3, basis_gates=["cx", "u3"])

    def rustiq_transpile(self, hamiltonian: SparsePauliOp):
        pauli_sequence = []
        for op in hamiltonian.paulis:
            pauli_sequence.append(op.to_label())  # type: ignore

        self.circuit = self.synthesis(pauli_sequence, hamiltonian.num_qubits) # type: ignore
...
class FermihedralMapper(FermionicMapper, ModeBasedMapper):
    ...
    def map( # type: ignore
        self, second_q_ops: FermionicOp, *, register_length: int | None = None
    ) -> SparsePauliOp:
