...
def qiskit_synthesis(ps_layers, time_parameter=1):
    from qiskit.aqua.operators.legacy import evolution_instruction # type: ignore
    ...
    for i in ps_layers:
        for j in i:
            for k in j:
                qc.append(evolution_instruction([[1, Pauli.from_label(k.ps[::-1])]], 1, 1), qc.qubits) # type: ignore
    return qc
...
def qiskit_synthesis1(ps_layers, time_parameter=1):
    ...
    for i in ps_layers:
        for j in i:
            for k in j:
                psl.append([1, Pauli.from_label(k.ps[::-1])]) # type: ignore
