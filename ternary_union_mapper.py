...
def _select_nodes(
    ...
    for xx, zz in tqdm(     # bash every permutation of xx, zz and greedily choose best
        ...
        vac = nqubits * 2
        desc = descendants[xx]
        if desc == vac:               # the VAC operator should have a mapping to only Z operators. This enforces that.
            continue

        if desc % 2 == 0:              #now the Y branch will be the corresponding other majorana operator
            pair = desc + 1
        else:
            pair = desc - 1
...
def _compile_fermionic_op(fermionic_op: FermionicOp, nqubits: int | None = None):
    ...
    for round in range(nqubits):
        ...
        selection = _select_nodes(terms, nodes, round, nqubits, tree, mapping, descendants, ancestors)
